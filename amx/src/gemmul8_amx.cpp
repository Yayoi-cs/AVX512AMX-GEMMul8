#include "../include/gemmul8_amx.hpp"

#include "common.hpp"
#include "inverse_scaling.hpp"
#include "matmult_amx.hpp"
#include "mod.hpp"
#include "scaling.hpp"
#include "table.hpp"
...
inline void run_scale_all<double>(Op op_A, Op op_B,
    ...
    switch (num_moduli) {
        ...
        default: throw std::invalid_argument("gemmul8_amx: num_moduli out of supported range [2, 20] for double");
    }
    switch (num_moduli) {
...
inline void run_scale_all_accurate<double>(Op op_A, Op op_B,
    ...
    matmult_amx::gemm_i8x1(w.A_lo_high, w.B_lo_high, w.C_hi, lo.m_pad,
                           m, n, lo.k_pad, lo.m_pad, lo.n_pad);

    std::vector<std::int32_t> amax_rows(m), amax_cols(n);
    matmult_amx::c_hi_row_absmax(w.C_hi, m, n, lo.m_pad, amax_rows.data());
    matmult_amx::c_hi_col_absmax(w.C_hi, m, n, lo.m_pad, amax_cols.data());
...
inline void run_scale_all<float>(Op op_A, Op op_B,
    ...
    switch (num_moduli) {
        ...
        default: throw std::invalid_argument("gemmul8_amx: num_moduli out of supported range [2, 7] for float");
    }
    switch (num_moduli) {
...
std::vector<double> gemm_impl(Op op_A, Op op_B,
    ...
    if (num_moduli < 2) throw std::invalid_argument("gemmul8_amx: num_moduli must be >= 2");
    ...
    auto t1 = tic();
    for (unsigned j = 0; j < num_moduli; ++j) {
        matmult_amx::gemm_i8x1(w.A_lo + j * lo.a_lo_bytes,
